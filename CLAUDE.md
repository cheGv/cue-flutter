...
> **Phase 4.0.8 spine (locked 8 May 2026, revised 9 May 2026):** typography and palette decisions are governed by [docs/design-language-spine-2026-05-08.md](docs/design-language-spine-2026-05-08.md) — read the original lock plus the **Revision 2026-05-09 (post-friend-tester signal)** section appended at the bottom. Sonoma surface + Logic Pro typography (Inter sans, JetBrains Mono mono, Iowan Old Style serif with Georgia fallback) + Things 3 warmth in editorial moments only. **Dual-accent system:** olive `#5C6E3B` is the calm/steady default UI accent (sidebar active state, brief-card stripe, "Today's move" labels, inline trial counts); amber `#B45309` is the urgent-register exception (yesterday-reminder, "Up next" indicator, primary actions). **Eyebrow doctrine:** mono uppercase tracked = data ONLY (dates, state pills, trial numbers); sans sentence-case = ALL human content labels (clinical card eyebrows, section headers, widget titles); sans uppercase tracked is FORBIDDEN except on state pills; Iowan serif = editorial moments + big plaque numerics (never Inter weight 600 for numerics — game-y register). Teal is removed; pre-4.0.8 teal sites migrate per-surface in the spine-doc order. The font/palette bullets below are pre-spine historical context — defer to the spine for any new code.
