...
> **Phase 4.0.8 spine (locked 8 May 2026, revised 9 May 2026 + 10 May 2026):** typography and palette decisions are governed by [docs/design-language-spine-2026-05-08.md](docs/design-language-spine-2026-05-08.md) — read the original lock plus the **Revision 2026-05-09 (post-friend-tester signal)** and **Revision 2026-05-10 (post-Roster surface 2 design)** sections appended at the bottom. Sonoma surface + Logic Pro typography (Inter sans, JetBrains Mono mono, Iowan Old Style serif with Georgia fallback) + Things 3 warmth in editorial moments only. **Dual-accent system:** olive `#5C6E3B` is the calm/steady default UI accent (sidebar active state, brief-card stripe, "Today's move" labels, inline trial counts); amber `#B45309` is the urgent-register exception (yesterday-reminder, "Up next" indicator, primary actions, the Roster's "Today" recency line). **Eyebrow doctrine (post-2026-05-10 narrowing):** mono uppercase tracked is for **data tags only** — state pills on clinical-task surfaces, date eyebrows, version/debug tokens. Inline numeric counts in human contexts (sessions, goals, ages) use **Inter weight 700 with tabular figures**, not mono — that's the "numbers as game changers" register. Iowan italic is reserved for **page identity moments** (page H1 + occasional editorial pull quotes); never for numerics. State pill register splits: clinical-task pills (Today brief cards) stay mono uppercase; **library-browse pills (Roster) are Inter sentence-case** — softer for scanning many rows. Cuttlefish placement carries across all primary surfaces: **64px `CueState.softWave` in an 80px left margin column** (Today, Roster, future Profile). New token `kCueGraySurface = #F1EFE8` for discharged / archived / deactivated registers (distinct from `kCueBorder`, which is hairline color only). Teal is removed; pre-4.0.8 teal sites migrate per-surface in the spine-doc order. The font/palette bullets below are pre-spine historical context — defer to the spine for any new code.
...
- **Domain Detector v1.3.2 — CLAUDE.md doctrine backlog.** Five doctrinal updates from the v1.3.1 spec's Task 7 are already operationalized in code (proxy `/cue-domain-detect` endpoint + migration applied 2026-05-13) but not yet documented in CLAUDE.md. Land in a follow-up commit after Evening 3 ships: (1) §6.X Clinical Domains — the nine-domain controlled vocabulary (dysphagia, aac, motor_speech, language, fluency, voice, aphasia, asd_regulatory, cognitive_communication) plus the two-bucket acute_clinical/developmental color mapping locked to the dual-accent spine. (2) §10 invariants append — the eight bullets from v1.3.1 Task 7 covering authority lock semantics, null-IFF-zero primary_domain pairing, app-layer history insert path, signal separation between failure and insufficient-input logs, and CHECK constraint symmetry. (3) §16.2 API Call Discipline. (4) §16.3 Signal Separation. (5) §16.4 Constraint Symmetry. Source of truth for content: `prompts/reviews/domain_detector_v1.3.2.md` in the proxy repo (`C:\dev\cue\proxy`). None of the five carries new behavior — all are documenting what already shipped.
