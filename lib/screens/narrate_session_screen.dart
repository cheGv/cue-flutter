...
class _NarrateSessionScreenState extends State<NarrateSessionScreen> {
  ...
  // ── Lifecycle ──────────────────────────────────────────────────────────
  // Phase 4.0.7.9f-part2 — port back the back-nav hydration that
  // c88380e introduced and that the revert undid. If the SLP backs out
  // and returns to a session that already has a saved transcript, we
  // surface it as the terminal stopped state so Generate Report is
  ...
  Future<void> _hydrateFromSession() async {
    try {
      ...
      if (saved != null && saved.isNotEmpty && mounted) {
        // Only hydrate if the user hasn't already started recording in
        // the gap between mount and this RPC returning — defends against
        // an init-race overwrite of a fresh take.
        if (_stage != _NarrateStage.idle) return;
        setState(() {
  ...
  Future<void> _startRecording() async {
    ...
    try {
      // ── Phase 4.0.7.9f-part2: fetch per-session keywords ───────────────
      // Pulls the keyword set (client name, current STG vocabulary, SLP
      // specializations, plus an SLP-universal baseline) from Supabase
      // and serializes it as a JSON-encoded query param the proxy will
      // sanitize and feed to Deepgram. Failure is non-fatal — we open
      // the WS without keywords rather than blocking the recording.
      String keywordsQuery = '';
      int keywordCount = 0;
      if (widget.sessionId != null) {
        try {
          _log('L3', 'fetching keywords via get_session_keywords RPC, '
              'sessionId=${widget.sessionId}');
          final raw = await _supabase.rpc(
            'get_session_keywords',
            params: {'p_session_id': widget.sessionId},
          );
          if (raw is List) {
            keywordCount = raw.length;
            final keywordsJson = jsonEncode(raw);
            keywordsQuery =
                '&keywords=${Uri.encodeQueryComponent(keywordsJson)}';
            _log('L3', 'fetched $keywordCount keywords for session, '
                'query_param_size=${keywordsQuery.length}');
          } else {
            _log('L3', 'keyword RPC returned non-list shape: ${raw.runtimeType}');
          }
        } catch (e) {
          _log('L3', 'keyword fetch failed (continuing without): $e');
        }
      }
